# Hi, I'm Théo Déliot 👋  
### 🎨 Creative Coder | Embedded Systems Enthusiast

💻 Student at 42 Berlin  
🔌 Passionate about electronics, robotics, and low-level programming  
🎯 Currently looking to join an embedded systems team to ship real hardware  

---

## 🚀 About Me
I started out prototyping interactive projects with microcontrollers in art school,  
and evolved into building larger-scale software systems at 42.

I enjoy working at the intersection of:
- ⚡ Hardware & software  
- 🎨 Creativity & engineering  
- 🧠 Systems thinking & real-world applications  

---

## 🛠️ Tech Stack
- **Languages:** C, C++, Python, MicroPython, TypeScript, SQL  
- **Tools:** Git, Docker / Docker Compose  
- **Focus:** Embedded systems, microcontrollers, prototyping  

---

## 🧪 Experience
**Lab Assistant — Jasoncook.io (2024)**  
- Taught and documented microcontroller projects  
- Built prototypes using MicroPython  
- Supported students in hands-on learning  

**Freelance Guitar Teacher**  
- One-on-one teaching & structured mentoring  

**Sales Associate — Cultura**  
- Client communication & problem-solving  

---

## 🎓 Education
- **42 Berlin — Core Curriculum (2022–Present)**  
  C / C++, systems programming, Docker  

- **Digital Systems — CNED** ★ 15/20  
- **Art School — ESAM** ★ 16/20  
- **Management & Economy — Saliège** ★ 16/20  

---

## 🌍 Languages
- 🇫🇷 French — Native  
- 🇬🇧 English — C1 (Fluent)  
- 🇩🇪 German — B1 (Basic)  

---

## 🤝 Let's Connect
I'm always interested in:
- Embedded systems projects  
- Creative tech collaborations  
- Building real-world hardware products  

📫 Reach out if you're working on something exciting!
